...
class DefenderOracle(nn.Module):
    def __init__(self, targetNum, featureCount):
        ...
        # Example defender observation
        #o_d = [1,0,0, 0,0,1, 0,0,1, 1.9,4.1,3.4]
...
def getMixedAttackerPolicy(game, payoffs):
    ...
    # First round
    tuple(np.concatenate(([1,0,0, 1,0,0, 0,0,0], payoffs))): [0,1,0],   #2
    tuple(np.concatenate(([1,0,0, 1,0,0, 1,0,0], payoffs))): [0,1,0],   #1

    # Second round
    tuple(np.concatenate(([0,1,0, 1,2,0, 0,0,0], payoffs))): [0,0,1],   #1
    tuple(np.concatenate(([0,1,0, 1,2,0, 0,1,0], payoffs))): [0,0,1],

    tuple(np.concatenate(([0,1,0, 1,2,0, 0,1,0], payoffs))): [0,0,1],   #2
    tuple(np.concatenate(([0,1,0, 1,2,0, 1,0,0], payoffs))): [0,0,1],
    }
...
def main():
    ...
    # Create Network
    # ==============
    model = DefenderOracle(3,4) # Create a defender oracle to train on a game with 3 targets and 4 features
    ...
    mixedAttackerPolicy = getMixedAttackerPolicy(game, rewards)
    ...
    while totalLoss > 1e-8:
        ...
        for _ in range(0,epochs):
            ...
            for timestep in range(game.timesteps):
                ...
                # Create model input
                aAction = mixedAttackerPolicy[tuple(aObservation)]  # Attacker action
                oldObservation = torch.from_numpy(game.previousDefenderObservation).float().requires_grad_(True)
                newObservation = torch.from_numpy(dObservation).float().requires_grad_(True)
                ...
                y, yScore = game.getBestActionAndScore(game.DEFENDER, aAction, rewards) # Defender action true label
                yVarBit = np.concatenate((game.previousDefenderAction,y))
    ...
    for i in range (0,15):
        ...
        for timestep in range(game.timesteps):
            ...
            # Create model input
            aAction = mixedAttackerPolicy[tuple(aObservation)]  # Defender action
            oldObservation = torch.from_numpy(game.previousDefenderObservation).float().requires_grad_(True)
            newObservation = torch.from_numpy(dObservation).float().requires_grad_(True)
            modelInput = torch.cat((oldObservation.unsqueeze(0),newObservation.unsqueeze(0)))
            ...
            y, yScore = game.getBestActionAndScore(game.DEFENDER, aAction, rewards) # Attacker action true label

            print(f"x guess: {x}")
            print(f"y:       {y}")

            game.performActions(x, aAction)
